...
def initialize_dialog():
    ...
    dialog = [
        {
       "role": "system",
            "content": f"You are the social robot Pepper, you have a tablet in your chest but it is only decoration. Let's have a warm, empathic, and casual conversation with your friend {user_name}."
                       ...
                       f"Have an engaging conversation, wherein you show interest in {user_name}. Ask open questions."
        }
    ]
# dialog = [
#         {
        ...
        try:
            ...
            if token_count >= MAX_TOKENS:  #When the tokens are over but the conversation still going, it is essential to make a summary about the current conversation.
                ...
                dialog = [
                    {
                        "role": "system",
                        "content": f"You are Pepper. Let's have a warm, empathic, and natural conversation with your friend {user_name}."
                                   f"We'll continue from the last conversation and include any relevant information. Previous conversation:  ###{last_conversation}###."
                                   f"The current date and time is {now.strftime('%m/%d/%Y %H:%M')} and we're speaking from {location}. Our conversation will be in {language}."
                                   f"Feel free to express yourself naturally, like a human friend!"
                    }
            ...
            answer = ask(message)
            client_socket.send("listening".encode())  # send message
